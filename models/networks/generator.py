...
class SesameGenerator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        input_nc = label_nc + 1 +3#label+mask+img #+ (1 if opt.contain_dontcare_label else 0) + (0 if opt.no_instance else 1)
    ...
    def forward(self, input_semantics, masked_image):

        input = torch.cat([input_semantics,masked_image], dim = 1)
