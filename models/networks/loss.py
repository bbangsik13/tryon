...
class GANLoss(nn.Module):
    ...
    def loss(self, input, target_is_real,target, for_discriminator=True):
        if self.gan_mode == 'original':  # cross entropy loss
            ...
            if for_discriminator:
                target_tensor = self.get_target_tensor(input, target)
                if target_is_real:
                    minval = torch.min(input - 1, target_tensor -1)
    ...
    def __call__(self, input, target_is_real,target, for_discriminator=True):
        ...
        if isinstance(input, list):
            ...
            for pred_i in input:
                if isinstance(pred_i, list):
                    pred_i = pred_i[-1]
                loss_tensor = self.loss(pred_i, target_is_real,target, for_discriminator)
