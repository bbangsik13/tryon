...
class SPADEResnetBlock(nn.Module):
    def __init__(self, fin, fout, opt, dilation=1):
        super().__init__()
        # Attributes
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, norm_layer, activation=nn.ReLU(False), kernel_size=3, dilation=1, dim2=None):
        super().__init__()
        ...
        self.conv_block = nn.Sequential(
            ...
            activation,
            nn.ReflectionPad2d(pw),
            norm_layer(nn.Conv2d(dim, dim2, kernel_size=kernel_size, dilation=dilation))
        )
