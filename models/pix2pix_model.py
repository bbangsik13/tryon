...
class Pix2PixModel(torch.nn.Module):
    ...
    def compute_discriminator_loss(self, input_semantics, real_image, masked_image):
        ...
        D_losses['D_Fake'] = self.criterionGAN(pred_fake,False, mask,
                                               for_discriminator=True)
        D_losses['D_real'] = self.criterionGAN(pred_real,True ,torch.cuda.FloatTensor(1).fill_(1.0).expand_as(mask),
                                               for_discriminator=True)
    ...
    def generate_fake(self, input_semantics, real_image, masked_image = None):
        ...
        fake_image = (1 - mask) * masked_image + mask * fake_image
        return fake_image
